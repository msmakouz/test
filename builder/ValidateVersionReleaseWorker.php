...
final class ValidateVersionReleaseWorker implements ReleaseWorkerInterface
{
    ...
    private function findMostRecentVersion(Version $version): ?string
    {
        ...
        $tags = \array_filter($tags, static fn (Version $tag) => static::isAllowedTag($version, $tag));
    ...
    private static function isAllowedTag(Version $requested, Version $tag): bool
    {
        // all previous major versions
        if ($requested->getMajor()->getValue() > $tag->getMajor()->getValue()) {
            return true;
        }

        // all minor versions up to the requested in the requested major version
        if ($requested->getMajor()->getValue() === $tag->getMajor()->getValue()) {
            return $requested->getMinor()->getValue() >= $tag->getMinor()->getValue();
        }
